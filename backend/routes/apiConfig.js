...
router.put('/siigo', auth.authenticateToken, adminOnly, async (req, res) => {
  try {
    ...
    // Encriptar access key y webhook secret
    const saltRounds = 12;
    const hashedAccessKey = await bcrypt.hash(siigo_access_key, saltRounds);
    const hashedWebhookSecret = webhook_secret ? await bcrypt.hash(webhook_secret, saltRounds) : null;
