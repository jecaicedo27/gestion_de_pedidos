...
class SiigoService {
  ...
  async processInvoiceToOrder(invoice, paymentMethod = 'transferencia', deliveryMethod = 'domicilio') {
    try {
      ...
      // Normalize payment method to match DB ENUMs
      const normalizedPaymentMethod = (() => {
        const pm = String(paymentMethod || '').toLowerCase();
        if (pm !== 'auto') return pm || 'efectivo';
        try {
          const name = String((fullInvoice.payments && fullInvoice.payments[0] && (fullInvoice.payments[0].name || fullInvoice.payments[0].payment_method || fullInvoice.payments[0].method)) || '').toLowerCase();
          if (name.includes('credito')) return 'credito';
          if (name.includes('efectivo') || name.includes('cash')) return 'efectivo';
          if (name.includes('transfer')) return 'transferencia';
          if (name.includes('tarjeta')) return 'tarjeta_credito';
          if (name.includes('pse') || name.includes('electr')) return 'pago_electronico';
        } catch (_) {}
        return 'efectivo';
      })();
  ...
  async getCustomers(page = 1, pageSize = 50) {
    try {
      ...
      return await this.makeRequestWithRetry(async () => {
        ...
        console.log(`✅ ${response.data.results?.length || 0} clientes obtenidos`);
        console.log(`📊 Total disponible: ${response.data.pagination?.total_results || 'N/A'}`);
      ...
      while (hasMorePages && page <= maxPages) {
        try {
          ...
          if (error.message.includes('429') || error.message.includes('rate')) {
            ...
            // Otros errores, detener la paginación
            hasMorePages = false;
          }
        }
      }
