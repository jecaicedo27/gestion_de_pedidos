...
class SiigoUpdateService {
  ...
  async logUpdate(invoiceId, orderId, status, errorMessage = null) {
    try {
      // Intentar con 'update' (para DBs que lo soportan)
      await query(`
      ...
      // Fallback para bases donde ENUM no incluye 'update'
      if (error && (error.code === 'WARN_DATA_TRUNCATED' || error.errno === 1265)) {
        try {
          await query(`
            ...
            VALUES (?, ?, 'manual', ?, ?, NOW())
          `, [invoiceId, orderId, status, errorMessage]);
          console.warn("ℹ️ Fallback aplicado: sync_type 'update' no soportado. Usando 'manual'.");
          return;
        } catch (fallbackError) {
          console.error('❌ Error registrando actualización (fallback manual):', fallbackError.message);
        }
      } else {
  ...
  async getUpdateStats() {
    try {
      const stats = await query(`
        ...
        FROM siigo_sync_log 
        WHERE sync_type IN ('update','manual')
