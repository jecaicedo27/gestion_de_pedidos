...
const OrderReviewModal = ({ isOpen, onClose, order, onConfirm }) => {
  ...
  const shippingPaymentMethods = [
    { value: 'contado', label: 'Contado (Pago ya realizado)' },
    { value: 'contraentrega', label: 'Contraentrega (Cobrar al entregar)' }
  ];
  ...
  const handleSendToLogistics = async () => {
    ...
    // Validar regla de negocio: transferencias deben ir a cartera primero
    if (formData.payment_method === 'transferencia') {
      toast.error('Los pagos por transferencia deben ir a cartera primero para verificar que el dinero esté en el banco');
      return;
    }
    ...
    try {
      ...
      await onConfirm(dataToSend);
      onClose();
      toast.success('Pedido enviado directamente a logística');
    } catch (error) {
  ...
  // Nueva función para procesamiento automático (solo para admin)
  const handleProcessOrder = async () => {
    ...
    try {
      ...
      if (formData.payment_method === 'efectivo') {
        // Efectivo va directo a logística
        targetDestination = 'logistics';
        actionType = 'send_to_logistics';
        successMessage = 'Pedido procesado y enviado a logística (pago en efectivo)';
      } else {
        // Todos los demás métodos van a cartera primero
      ...
      const dataToSend = {
        ...
        notes: formData.notes,
        action: actionType,
        auto_processed: true // Marcar que fue procesado automáticamente
      };
  ...
  // Nueva función para eliminar pedido de SIIGO
  const handleDeleteSiigoOrder = async () => {
    if (!window.confirm('¿Está seguro que desea eliminar este pedido? Esta acción eliminará completamente el pedido y volverá a estar disponible para importar desde SIIGO.')) {
      return;
    }

    setLoading(true);
    try {
      const response = await fetch(`/api/orders/${order.id}/siigo`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      ...
      if (response.ok) {
        ...
        toast.success(result.message || 'Pedido eliminado exitosamente');
        onClose();
        // Refrescar la página o lista de pedidos
        window.location.reload();
      } else {
        const error = await response.json();
        toast.error(error.message || 'Error eliminando el pedido');
      }
    } catch (error) {
      console.error('Error eliminando pedido:', error);
      toast.error('Error de conexión al eliminar el pedido');
    } finally {
      setLoading(false);
    }
  };
  ...
  const getRecommendedAction = () => {
    ...
    // Recomendación basada en reglas de negocio
    if (formData.payment_method === 'efectivo') {
      return {
        action: 'logistics',
        reason: 'Pago en efectivo - Se puede enviar directamente a logística'
      };
    } else if (formData.payment_method === 'transferencia') {
      return {
        action: 'wallet',
      ...
      return {
        action: 'wallet',
        reason: 'Pago no efectivo - Requiere validación de cartera'
      };
    }
  };
