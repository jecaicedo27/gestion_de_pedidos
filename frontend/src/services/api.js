 ...
 */
/**
 * Determina la baseURL de la API.
 * - En producción (no localhost), forzamos same-origin '/api' para evitar builds mal configurados (p. ej. REACT_APP_API_URL="http://api").
 * - En desarrollo, si existe REACT_APP_API_URL válida se usa; de lo contrario, '/api'.
 */
const resolveApiBaseUrl = () => {
  ...
  // En producción siempre same-origin
  if (isBrowser && !isLocalhost) {
    return '/api';
  }

  // En dev: permitir env var si es válida; si no, '/api'
  if (!envBase) return '/api';
  if (envBase.startsWith('/')) return envBase;

  try {
    // Valida URL absoluta http(s)
    // new URL lanzará si es inválida (evita casos como "api" sin dominio)
    // Nota: si el host no resuelve en DNS, fallará en runtime (aceptable en dev)
    // Para dev preferimos respetar la URL si es sintácticamente válida.
    // Ejemplos válidos: http://localhost:3001/api, http://127.0.0.1:3001/api
    // Ejemplos inválidos (se ignoran): "api", "http://"
    // Si quieres forzar una IP/Dominio, usa REACT_APP_API_URL completa.
